...
public class GUI implements ActionListener, MyConstants{
	private final JFrame jFrame = new JFrame();
    private final BorderLayout borderLayout = new BorderLayout();
    private final JPanel leftPanel = new JPanel();
    private final JPanel rightPanel = new JPanel();
    private final JPanel bottomPanel = new JPanel();
    private final TitledBorder leftBorder = new TitledBorder("Info");
    private final TitledBorder rightBorder = new TitledBorder("Control");
    private final TitledBorder bottomBorder = new TitledBorder("Configuration");
	...
	public void actionPerformed(ActionEvent actionEvent) {
		if (actionEvent.getActionCommand().equals(START_SERVER)) {
			if(Main.setRootDirectory(jTextField1.getText())
					&& Main.setMaintenanceDirectory(jTextField2.getText())) {
				jTextField1.setForeground(BLACK);
				jTextField2.setForeground(BLACK);
				Main.setServerSocket((Integer) jSpinner.getValue());
	    	...
	    	if(Main.setMaintenanceDirectory(jTextField2.getText())) {
	    		Main.setServerIsInMaintenance(true);
	    		jTextField2.setForeground(BLACK);
	    	...
	    	if(Main.setRootDirectory(jTextField1.getText())) {
	    		Main.setServerIsInMaintenance(false);
	    		jTextField1.setForeground(BLACK);
		    	jFrame.setTitle(TITLE_WHILE_RUNNING);
				jLabel4.setText(RUNNING);
				jCheckBox.setActionCommand(START_MAINTENANCE);
